...
<div class="content">
    ...
    <div class="summary-cards">
        <div class="summary-card">
            <h5>Всего бюджет</h5>
            <p><?= number_format($summary['total_budget'] ?? 0, 2) ?></p>
        </div>
        <div class="summary-card">
            <h5>Потрачено</h5>
            <p style="color:#dc2626;"><?= number_format($summary['total_spent'] ?? 0, 2) ?></p>
        </div>
        <div class="summary-card">
            <h5>Остаток</h5>
            <p style="color:#16a34a;"><?= number_format($summary['remaining'] ?? 0, 2) ?></p>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
                ...
                fetch(`${viewUrl}?id=${id}`)
                    ...
                    .then(data => {
                        if (data.success && data.budget) {
                            ...
                            form.querySelector('[name="Budget[amount]"]').value = b.amount;
                            ...
                            alert(data.message || 'Ошибка при загрузке бюджета');
                        }
                    })
            ...
            btn.addEventListener('click', () => {
                ...
                fetch(deleteUrl, {
                    ...
                    .then(data => {
                        ...
                        else alert(data.message || 'Ошибка при удалении');
                    })
                    .catch(err => console.error(err));
            });
        });
        ...
        document.querySelector('.saveBudget').addEventListener('click', () => {
            ...
            fetch(url, {
                ...
                .catch(err => {
                    console.error(err);
                    formErrors.textContent = 'Произошла ошибка при отправке формы';
                });
        });
    });
</script>
